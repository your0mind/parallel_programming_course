...
int main(int argc, char** argv) {
    ...
    tbb::task_scheduler_init init(nNumThreads);
    ...
    if (argc == 5) {
        ...
        FillArray(pdArr, nSizeArr, -10000000, 100000000);
    }
    ...
    std::cout << "Time of work parallel version: " << (pt2 - pt1).seconds() << " sec." << std::endl;
    ...
    std::cout << "Time of Work serial version: " << (st2 - st1).seconds() << " sec." << std::endl;
    ...
    cout << "Speed up: " << (st2 - st1).seconds() / (pt2 - pt1).seconds() << std::endl;
    delete[] pdArr;
    delete[] pdCopyArr;
    delete[] pdResArr;
    return 0;
...
void ShellMerge(Type* pArr, uint32_t nSizeArr, Type* pResArr, int32_t nNumThreads) {
    if (nNumThreads == 1) {
        ...
        {
            tbb::task_group g;
            g.run([&pArr, &pResArr, nSizeArr, nNumThreads] {
