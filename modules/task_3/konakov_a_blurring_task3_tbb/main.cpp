...
int tbb_pgm_image_convolution_filter(pgm_image *img_dest, pgm_image *img_src,
                                     double *conv_mat = nullptr,
                                     int cm_size = 5, const int threads = 8) {
  pgm_image_create(img_dest, img_src->height, img_src->width, PGM_IMAGE_BLACK);

  // !!!!
  tbb::parallel_for(
      tbb::blocked_range2d<int>(0, img_src->width, img_src->width / threads, 0,
                                img_src->height, img_src->height / threads),
      [&](tbb::blocked_range2d<int> &range) {
        int r, c;
        for (r = range.rows().begin(); r < range.rows().end(); ++r)
          for (c = range.cols().begin(); c < range.cols().end(); ++c)
            for (int cmr = 0; cmr < cm_size; ++cmr)
              for (int cmc = 0; cmc < cm_size; ++cmc)
                img_dest->pixels[r * img_dest->width + c] +=
...
int pgm_image_gaussian_smooth(pgm_image *img_dest, pgm_image *img_src,
  ...
  int ret_val = ((mode == ser) ? pgm_image_convolution_filter(
                                     img_dest, img_src, gauss_mat, ker_size)
                               : tbb_pgm_image_convolution_filter(
                                     img_dest, img_src, gauss_mat, ker_size));
  delete[] gauss_mat;
  return ret_val;
}
...
int main(int argc, char **argv) {
  ...
  tbb::tick_count tbbStart = tbb::tick_count::now();  // omp_get_wtime();
  pgm_image_gaussian_smooth(smoothed_ser, gen_img, sigma, ker_size);
  double ser_time = (tbb::tick_count::now() - tbbStart).seconds();
  std::cout << "Serial time: " << ser_time << " sec\n";

  tbb::task_scheduler_init init(threads);

  tbbStart = tbb::tick_count::now();
  pgm_image_gaussian_smooth(smoothed_ser, gen_img, sigma, ker_size, par);
  double par_time = (tbb::tick_count::now() - tbbStart).seconds();
  std::cout << "Parallel time(" << threads << " threads):" << par_time << " sec"
            << std::endl;
